...
function manager:init(args)
    self.mods={}
    self.mod_dir=args.mod_dir or mod_dir
    local mods=self.mods
    local mlist=dfhack.internal.getDir(self.mod_dir)

    if #mlist==0 then
        qerror("Mod directory not found! Are you sure it is in:"..self.mod_dir)
    end
    for k,v in ipairs(mlist) do
        if v~="." and v~=".." and v~="init.lua" then
            local f,modData=pcall(dofile,self.mod_dir.."/".. v .. "/init.lua")
            if f then
                mods[modData.name]=modData
                modData.directory=self.mod_dir..'/'..v
    ...
    self:addviews{
        ...
        widgets.Panel{subviews={
            ...
            widgets.Label{
                ...
                {text="Open category",key="CUSTOM_O",key_sep="()",enabled=self:callback("isCategory"),
                    on_activate=self:callback("openCategory")},NEWLINE,
...
function manager:isCategory()
    return self.selected.category
end
function manager:openCategory()
    print(self.selected.directory)
    local c=manager{mod_dir=self.selected.directory}
    c:show()
end
